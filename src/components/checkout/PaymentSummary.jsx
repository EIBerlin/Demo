...
export default function PaymentSummary() {
  ...
  const handlePayment = async () => {
    try {
      ...
      const cryptoAmount = method === 'crypto' ? details.cryptoPrice : details.usdPrice;

      const API_BASE = process.env.REACT_APP_API_URL || 'http://localhost:4000';

      await fetch(`${API_BASE}/api/data/purchase_history`, {
        method: 'POST',
      ...
      await fetch(`${API_BASE}/api/data/reward_history`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          user_email: user.email,
